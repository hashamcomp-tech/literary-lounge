  ...
  const processManuscriptPaste = (rawText: string) => {
    ...
    if (!existing) {
      ...
      toast({
        ...
        description:
          "Couldn't match the first line to an existing series. Fields were not auto-filled — please fill them in manually.",
      });
      return;
    }
      ...
      if (sourceMode === "file" && selectedFile) {
        if (selectedFile.name.toLowerCase().endsWith(".epub")) {
          ...
          for (const spineId of spineIds) {
            ...
            const rawText = chapterDoc.body?.textContent?.trim() ?? "";
            if (!rawText) continue;
          ...
          <form onSubmit={handleUpload} className="space-y-5">
            ...
            <div className="space-y-1.5 relative">
              ...
              {showSuggestions && (
                <div
                  ref={suggestionRef}
                  className="absolute z-50 w-full mt-1 bg-popover border rounded-xl shadow-lg overflow-hidden"
                >
              ...
              <TabsContent value="file" className="mt-3">
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    <label
                      htmlFor="file"
                      className="flex-1 flex items-center gap-2 border rounded-xl px-3 py-2 cursor-pointer hover:bg-accent transition-colors text-sm text-muted-foreground"
                    >
