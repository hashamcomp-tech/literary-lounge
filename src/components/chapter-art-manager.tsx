...
export function ChapterArtManager({
  bookId,
  firestore,
  currentChapterNum,
  onClose,
  onArtUpdated
}: ChapterArtManagerProps) {
  ...
  const loadArt = async () => {
    ...
    try {
      const snap = await getDocs(
        collection(firestore, 'books', bookId, 'chapterArt')
      );

      const list: ChapterArt[] = snap.docs.map(
        (d) =>
          ({
            id: d.id,
            ...d.data()
  ...
  const handleUpload = async (
    ...
    try {
      const optimized = await optimizeCoverImage(
        file,
        1400,
        selectedAspectRatio
      );
  ...
  const handleDelete = async (art: ChapterArt) => {
    if (
      !confirm(
        `Delete "${art.name}"? This will remove it from all assigned chapters.`
      )
    ) {
      return;
    }
    ...
    try {
      await deleteDoc(
        doc(firestore, 'books', bookId, 'chapterArt', art.id)
      );

      try {
        const { deleteFromVercelBlob } = await import(
          '@/app/actions/upload'
        );
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      if (assignMode === 'current') {
        ...
        if (!parsed.length) {
          alert('No valid chapters entered.');
          setSaving(false);
          return;
        }
      ...
      await updateDoc(
        doc(firestore, 'books', bookId, 'chapterArt', selectedArt.id),
        {
          assignedChapters: chapters
        }
      );

      const updated = artList.map((a) =>
        a.id === selectedArt.id
          ? { ...a, assignedChapters: chapters }
          : a
      );
  ...
  const handleUnassignCurrent = async (art: ChapterArt) => {
    ...
    try {
      const chapters = art.assignedChapters.filter(
        (n) => n !== currentChapterNum
      );
  ...
  const isAssignedToCurrent = (art: ChapterArt) =>
    art.assignedChapters.includes(currentChapterNum);
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-border/50 shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Upload art once, assign to any chapters
            </p>
          </div>
                ...
                {artList.map((art) => {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        'relative rounded-2xl overflow-hidden cursor-pointer transition-all border-2',
                        getAspectRatioClass(art.aspectRatio),
                        isSelected
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-black/40 flex flex-col justify-between p-2">
                        ...
                        <div className="flex justify-end gap-1 flex-wrap">
                          {assigned && (
                            <span className="bg-primary text-primary-foreground text-[9px] font-black uppercase px-2 py-0.5 rounded-full">
                          ...
                          <div className="min-w-0">
                            {art.assignedChapters.length > 0 && (
                              <span className="block bg-black/60 text-white/80 text-[8px] font-mono px-1.5 py-0.5 rounded mb-1 truncate">
                                {chaptersToRangeString(
                                  art.assignedChapters
                                )}
                              </span>
                            )}
          ...
          {selectedArt && (
            <div className="mx-6 mb-6 rounded-2xl border border-border bg-muted/20 p-4 space-y-4 animate-in slide-in-from-bottom-2 duration-200">
              ...
              <div className="flex items-center gap-3">
                ...
                <div
                  className={cn(
                    'w-20 shrink-0 overflow-hidden rounded-lg',
                    getAspectRatioClass(
                      selectedArt.aspectRatio
                    )
                  )}
                >
                  <img
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className={cn(
                    'px-4 py-2 rounded-full text-[11px] font-black uppercase tracking-widest border transition-all',
                    assignMode === 'current'
                ...
                {isAssignedToCurrent(selectedArt) && (
                  <button
                    onClick={() =>
                      handleUnassignCurrent(selectedArt)
                    }
                    disabled={saving}
