...
/**
 * Updates basic bibliographic metadata and narrative summary for a cloud novel.
 */
export async function updateCloudBookMetadata(
  db: Firestore, 
  bookId: string, 
  updates: { title: string, author: string, summary?: string }
) {
  const bookRef = doc(db, 'books', bookId);
  const payload = {
    title: updates.title,
    ...
    'metadata.info.summary': updates.summary || "",
    lastUpdated: serverTimestamp()
  };
